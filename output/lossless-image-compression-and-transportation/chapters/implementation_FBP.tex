...
\[
 \text{net}_{pj}^{L} = \sum_{i=1}^{n_{L}} w_{ij}^{L} \cdot \text{out}_{i}^{L} + \text{bias}_{j}^{L}, \cite{sarkar1995, wasserman1989, Khaleel2014}
\]
where $p$ indexes the pattern, $w_{ij}^{L}$ are the weights, and $\text{out}_{i}^{L}$ is the output from the previous layer. The activation output is computed using a sigmoid function \cite{sarkar1995, wasserman1989, VemuriSahniChenKapoorLeonardFitzsimmons}:
\[
...
For the output layer, the error signal is \cite{sarkar1995, wasserman1989, VemuriSahniChenKapoorLeonardFitzsimmons}:
\[
...
\[
\Delta w_{ij}^{L} = z_i \cdot \eta \cdot \delta_{pj}^{L-1} \cdot \text{out}_{pj}^{L}, \cite{pedrycz1996fuzzy, VemuriSahniChenKapoorLeonardFitzsimmons}
\]
...
\begin{table}[H]
\centering
\caption{Training loss convergence for the Fuzzy Backpropagation Neural Network.}
...
\begin{itemize}
    \item The original Lenna image used for training and testing.
    \item The FBPNN compressed image, strictly utilizing a 15-colour palette.
    \item An absolute error map, where brighter regions are used to indicate a larger reconstruction error.
    \item The training loss convergence curve, illustrating the smooth decline in error across epochs.
    \item A summary of compression statistics, including bit rate and total size reduction.
\end{itemize}
