...
\]
where $p$ indexes the pattern, $w_{ij}^{L}$ are the weights, and $\text{out}_{i}^{L}$ is the output from the previous layer. The activation output is computed using a sigmoid function \cite{sarkar1995, Wasserman1989, Khaleel2014}:
\[
...
For the output layer, the error signal is \cite{sarkar1995, Wasserman1989, Khaleel2014}:
\[
...
\[
\Delta w_{ij}^{L} = z_i \cdot \eta \cdot \delta_{pj}^{L-1} \cdot \text{out}_{pj}^{L}, \cite{sarkar1995, Khaleel2014}
\]
