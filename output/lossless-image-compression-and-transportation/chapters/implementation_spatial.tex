...
\section{Variable Block Size Compression with Adaptive Encoding}
...
\subsubsection{Local Complexity Estimation}
...
\subsubsection{Adaptive Block Size Selection}
...
\text{Block size} = 
\begin{cases}
32\times32 & \text{if } \rho(B) < \theta_1 \quad (\text{smooth region}),\\[4pt]
16\times16 & \text{if } \theta_1 \le \rho(B) < \theta_2 \quad (\text{moderate detail}),\\[4pt]
8\times8   & \text{if } \rho(B) \ge \theta_2 \quad (\text{high detail}).
\end{cases}
\]

For the Lenna image, the thresholds are set to $\theta_1 = 0.1$ and $\theta_2 = 0.2$ (normalised gradient magnitude). This yields approximately 35\% large blocks, 30\% medium blocks, and 35\% small blocks, as illustrated in Figure~\ref{fig:block_distribution}.
...
\paragraph{Run‑Length Encoding (RLE)} For smooth, nearly uniform blocks ($\rho(B) < 0.1$), RLE is applied. The block is represented as a sequence of tuples $(value, run)$. For a block of size $s \times s$, the number of bits required is:
...
\subsection{Experimental Results on Lenna Image}
...
The distribution of encoding methods (Figure~\ref{fig:encoding_pie}) shows that 35\% of blocks are encoded with RLE, 30\% with Base+Offset, 15\% with block matching, and 20\% are left raw. This reflects the heterogeneous nature of the Lenna image and validates the adaptive selection logic.
