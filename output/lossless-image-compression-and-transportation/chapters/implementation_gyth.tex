...
The distance measure incorporates cluster shape information via \cite{hasanzadeh2005, Khaleel2014} the fuzzy covariance matrix $F_{mi}$:
...
where $P_i$ is the prior probability of cluster $i$, computed as the average membership \cite{hasanzadeh2005}:

\[
P_i = \frac{1}{N} \sum_{k=1}^{N} \mu_{ik}. 
\]

The fuzzy covariance matrix is defined as \cite{hasanzadeh2005, Khaleel2014}:

\[
F_{mi} = \frac{\sum_{k=1}^{N} (\mu_{ik})^m (z_k - v_i)(z_k - v_i)^T}{\sum_{k=1}^{N} (\mu_{ik})^m}.
\]

The cluster centres are updated using \cite{hasanzadeh2005, Khaleel2014}:

\[
v_i = \frac{\sum_{k=1}^{N} (\mu_{ik})^m z_k}{\sum_{k=1}^{N} (\mu_{ik})^m}. 
\]

Finally, the membership degrees are recomputed as \cite{hasanzadeh2005, Khaleel2014}:

\[
\mu_{ik} = \frac{1}{\sum_{j=1}^{c} \left( \frac{D_{ik}}{D_{jk}} \right)^{\frac{2}{m-1}}}.
\]

The algorithm iteratively updates centres, covariances, and memberships until convergence (i.e., when the change in $U$ falls below a threshold $\epsilon$) or a maximum number of iterations is reached. \cite{Khaleel2014}
...
\hline
\textbf{Image} & \textbf{Clusters $K$} & \textbf{Time (s)} & \textbf{Speed (pts/s)} & \textbf{Compression Ratio} \\
\hline
Lenna & 2  & 0.09 & 711,017 & — \\
Lenna & 4  & 0.17 & 379,886 & — \\
Lenna & 8  & 0.64 & 102,187 & 7.99:1 \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{gath_effect_leena.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{gath_chart_leena.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{gath_clust_leena.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{gath_camera_chart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{gath_camera_compare.png}
    \caption{Compression ratio vs. number of clusters. The gain saturates after $K=8$.}
...
\begin{itemize}
    \item \textbf{Advantages:} Gath‑Geva can model ellipsoidal clusters of varying sizes and densities, making it more flexible than spherical‑only algorithms. It provides fuzzy memberships that capture uncertainty. \cite{Khaleel2014}
    \item \textbf{Limitations:} The algorithm is computationally more expensive than K‑Means or FCM, especially for large datasets and high numbers of clusters. The need to compute and invert covariance matrices for each cluster at every iteration dominates the runtime. \cite{Khaleel2014}
