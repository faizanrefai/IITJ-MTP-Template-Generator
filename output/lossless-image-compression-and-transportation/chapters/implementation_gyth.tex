...
\[ \cite{gath1989, VemuriSahniChenKapoorLeonardFitzsimmons}
...
\[
P_i = \frac{1}{N} \sum_{k=1}^{N} \mu_{ik}. \cite{gath1989}
\]

The fuzzy covariance matrix is defined as \cite{gath1989, VemuriSahniChenKapoorLeonardFitzsimmons}:
...
\[
v_i = \frac{\sum_{k=1}^{N} (\mu_{ik})^m z_k}{\sum_{k=1}^{N} (\mu_{ik})^m}. \cite{gath1989, Khaleel2014}
\]

Finally, the membership degrees are recomputed as \cite{gath1989, VemuriSahniChenKapoorLeonardFitzsimmons}:
...
\begin{itemize}
    \item \textbf{Advantages:} Gath‑Geva can model ellipsoidal clusters of varying sizes and densities, making it more flexible than spherical‑only algorithms. It provides fuzzy memberships that capture uncertainty. \cite{VemuriSahniChenKapoorLeonardFitzsimmons}
